...
Created by: Binh Nguyen, John Nguyen, & Kaytlin Simmers
"""
...
def main():
    ...
    print('Please type desired image augmentation option(s)\n(If multiple selected, separate by space)\n')
    augment = input('elasticdeformation/rotation/horizontalrotation/verticalrotation/none\n\n').split()
    augment_list = list(map(str,augment))

    for x in augment_list:
            print("x value",x)
    if x !='elasticdeformation' and x != 'rotation' and x != 'horizontalrotation' and x != 'verticalrotation' and x != 'none':
            print('This is not a valid image augmentation, exiting the program')
            exit(0)
    if "rotation" in augment_list:
            degree = int(input('\nEnter desired rotation degree: '))
